...
SETUP_GUIDES: dict[str, str] = {
    ...
    "outlook_graph": (
        ...
        "  - If you get AADSTS16000, or 'The ability to create applications "
        "outside of a directory has been deprecated', you have no directory "
        "yet. Microsoft offers two ways to get one:\n"
        "      a) Join the M365 Developer Program - NOTE this now requires a "
        "PAID Visual Studio Professional or Enterprise subscription (monthly "
        "VS subscriptions do not qualify), so it is not a free option for "
        "most people.\n"
        "      b) Sign up for the free Azure account at azure.microsoft.com "
        "with this same Microsoft account. That creates a 'Default Directory'. "
        "A card is required for identity verification; the free tier is not "
        "charged, and app registration stays on the always-free Entra tier "
        "afterwards. Then return to entra.microsoft.com.\n"
        "    A business Microsoft 365 account already HAS a directory, so at a "
        "customer site this whole step usually does not apply.\n"
        ...
        "sign in without it.\n"
        "4. Redirect URI: leave blank for now. Select Register.\n"
        ...
        "blank.\n\n"
        "STEP 3 - allow device-code sign-in (BOTH parts needed)\n"
        "In the app go to Manage > Authentication:\n"
        "  3a. 'Allow public client flows' (under Advanced settings) = YES, "
        "then Save. If you cannot find it, open Manage > Manifest and set "
        "\"allowPublicClient\": true instead.\n"
        "  3b. Add a platform > 'Mobile and desktop applications' > tick "
        "https://login.microsoftonline.com/common/oauth2/nativeclient > "
        "Configure. A PERSONAL Microsoft account will not complete "
        "device-code sign-in without a registered redirect URI - you get "
        "\"invalid_request: The provided request must include a 'redirect_uri' "
        "input parameter\" without it.\n\n"
